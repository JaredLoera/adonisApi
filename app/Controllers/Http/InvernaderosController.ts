...
const uri =
  'mongodb+srv://abel0120:01abel01@cluster0.mndru6r.mongodb.net/?retryWrites=true&w=majority'
...
export default class InvernaderosController {
  ...
  async getDatosSen({ response, params }) {
    let client
    try {
      ...
      const { nombre, sensor } = params

      const sensores = await collection
        .aggregate([
          {
            $match: {
              nombre: sensor,
  ...
  async getDatosFechaAntigua({ response, params }) {
    let client
    try {
      client = await MongoClient.connect(uri)
      const database = client.db('sistemaSensores')
  ...
  async getDatosMenorMayor({ response, params }) {
    let client
    try {
      client = await MongoClient.connect(uri)
      const database = client.db('sistemaSensores')
  ...
  async getDatosMayorMenor({ response, params }) {
    let client
    try {
      client = await MongoClient.connect(uri)
      const database = client.db('sistemaSensores')
