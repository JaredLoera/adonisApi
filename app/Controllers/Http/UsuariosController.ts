...
export default class UsuariosController {
  ...
  public async createUser({ request, response }) {
    ...
    if (await user.save()) {
      ...
      await Mail.send((message) => {
        message.to('lujanlom0120@gmail.com').from('sender@example.com').subject('Welcome')
  ...
  public async logoutUsuario({ response, auth }: HttpContextContract) {
  ...
  public async verificarUsuarioSMS({ params, request, response }) {
    try {
      ...
      const { error } = validationSchema.validate(request, { abortEarly: false })
      if (error) {
        const errorMessages = error.details
        return response.badRequest({ error: errorMessages })
      }
      const user = await User.findBy('id', params.id)
  ...
  public async getAdmin({ response }: HttpContextContract) {
    const user = await Database.query()
      .from('users')
      .select(
        'users.id as id',
  ...
  public async getUserEmail({ response, params }: HttpContextContract) {
    const { email } = params
    const user = await Database.query()
      .from('users')
      .select(
  ...
  public async getUserName({ response, params }: HttpContextContract) {
    ...
    if (user.length === 0) {
      return response.status(404).json({
        status: 404,
        mensaje: 'Usuario no encontrado',
        error: 'No se encontró un usuario con el nombre proporcionado',
        data: [],
      })
    }
