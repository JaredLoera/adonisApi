...
Route.group(() => {
  ...
  Route.get('/mostrar/inverdatos/:nombre', 'InvernaderosController.getInverSen').where(
    'nombre',
    /^[0-9a-zA-Z]+$/
  )

  Route.get('/mostrar/sensordatos/:sensor/:nombre', 'InvernaderosController.getDatosSen').where(
    'nombre',
    /^[0-9a-zA-Z]+$/
  )

  Route.get('/mayormenor/:sensor/:nombre', 'InvernaderosController.getDatosMenorMayor').where(
    'nombre',
    /^[0-9a-zA-Z]+$/
  )
  Route.get('/menormayor/:sensor/:nombre', 'InvernaderosController.getDatosMayorMenor').where(
    'nombre',
    /^[0-9a-zA-Z]+$/
  )
  Route.get('/fechantigua/:sensor/:nombre', 'InvernaderosController.getDatosFechaAntigua').where(
    'nombre',
    /^[0-9a-zA-Z]+$/
  )
})
...
Route.group(() => {
  ...
  Route.get('/filtrouser', 'UsuariosController.getUser')
  Route.get('/filtroadmin', 'UsuariosController.getAdmin')
  Route.get('/buscaremail/:email', 'UsuariosController.getUserEmail')
  Route.get('/buscarname/:email', 'UsuariosController.getUserName')
